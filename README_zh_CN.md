...
- **批量导入**：选择文件夹，递归上传资源（md 引用的图片、html 的 `_files` 资源文件夹），然后逐个文件转换并导入
- **资源支持**：md 引用的本地图片和 html 的 `_files` 资源文件夹会自动上传并导入（单个导入和批量导入均支持）
- **无感刷新**：导入完成后文档树自动刷新，无需刷新页面
- **同名冲突处理**：当转换源（如 `a.docx`）与已存在的 md 文件（如 `a.md`）同名时，转换产物自动改名（如 `a-docx-1.md`），不会覆盖任何文件
- **临时文件清理**：一键清理覆盖转换目录和安全目录；导入前检测到残留临时文件会先确认清理
- **临时文件夹路径**：按需展示转换目录和安全目录路径，点击某个路径可直接在文件管理器中打开
...
**1.12.0 (2026-08-25)**
### Features
* 新增可选「导入图片迁移到资料库（assets）」设置（[#231](https://github.com/terwer/siyuan-plugin-importer/issues/231)）。开启后，md 中引用的本地图片（含绝对路径）交由思源接管，最终进入 `data/assets`，命名为 `<原名>_<父目录hash>.<ext>`（附带内核随机后缀），同名图片自动去重、图片可正常显示。默认关闭。

**1.11.0 (2026-08-24)**
### Features
* 导入含网络图片（http/https）的 md 后，插件会自动检测并提示你用思源内置「网络资源文件转换本地」（文档右上角「···」），无需自己下载
### Bug Fixes
* md 导入支持自定义处理函数：此前 `.md` 文件直接上传、跳过自定义处理函数，启用后没有任何反馈；现在会在上传前对 md 文本执行自定义函数（仍不做内置处理，避免改动原始 md）

**1.10.0 (2026-08-24)**
### Features
* 给从 docx 导入的 HTML 表格添加边框：含合并单元格(colspan/rowspan)的复杂表格被渲染为 HTML 块时默认无边框，导入时自动注入边框样式
### Bug Fixes
* 切换 pandoc 输出格式为 markdown，修复 Word 导入问题：含合并单元格的复杂表格被渲染为字面 `[table]` 占位导致表格内容丢失；公式被渲染为 `math` 代码块

**1.9.0 (2026-08-09)**
### Features
* 改为逐个文件导入转换后的 md，不再整目录导入，规避了内核死锁导致的重复导入必卡死问题
* 导入完成后文档树自动刷新（无感，不刷新页面）
* html 导入支持 `_files` 资源文件夹（单个导入和批量导入均支持）
* md 导入支持引用的本地图片（导入前自动上传）
* 转换产物与已有 md 文件同名时自动改名（如 `a.docx` + `a.md` → `a-docx-1.md`），不会覆盖任何文件
* 临时文件清理覆盖转换目录和安全目录；导入前检测到残留临时文件会先确认清理
* 临时文件夹路径按需展示，点击某个路径可直接在文件管理器中打开
### Bug Fixes
* 修复思源 3.7.3+ 内核拒绝 `temp/convert` 下路径（仅允许 `temp/export/*`）导致的导入失败；转换文件导入前先复制到安全路径
* 修复目录导入路径分隔符不匹配导致的内核卡死
* 修复临时目录残留文件导致的重复导入
